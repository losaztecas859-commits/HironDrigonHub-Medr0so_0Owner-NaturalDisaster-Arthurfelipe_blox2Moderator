...
"https://raw.githubusercontent.com/losaztecas859-commits/Redz-library-v5.docs/refs/heads/main/testee2.lua"
))()
local Window = Libary:MakeWindow({
    Title = "HironDrigon Hub | Brookhaven RP 🏡",
    SubTitle = "by: Medr0so_0",
    LoadText = "HironDrigon ARQUIVO",
    Flags = "HironDrigon hub_Broookhaven"
})
...
InfoTab:AddParagraph({ "Colaborações:", "HironDrigon Team" })
InfoTab:AddParagraph({ "Você está usando:", "HironDrigon Hub" })
InfoTab:AddParagraph({"Versão do brookhaven em BETA!"})
InfoTab:AddParagraph({"Novidades:", "Lag com ônibus já está disponível!"})
InfoTab:AddButton({"VERSÃO EDIT", function()
    ...
    Callback = function()
        if not Target then 
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Erro do caralho",
                Text = "Seleciona um player primeiro, porra!",
                Duration = 5
            })
            return 
        end
...
function AvatarManager:Equipar(nome)
    ...
    AvatarManager:Notificar("Não achei essa merda")
end
...
Tab:AddButton({
    ...
    Callback = function()
        if _G.Selected3D then
            ...
            AvatarManager:Notificar("Escolhe alguma coisa antes, viado!")
        end
    end
})
...
local Tab = Window:MakeTab({"Teleportes", "tp"}) -- CHAOS do ícone: 'portal' em minúsculo para teleporte
...
protec:AddSection({ Name = "MENU ANTI" })
        ...
        if rodando then
            ...
            task.spawn(function()
                while rodando do
                    ...
                    -- Remove suas próprias tools localmente
                    pcall(function()
...
-- Função genérica pra anti-fling de veículos
local function AntiFlingLoop(name, getFolderFunc)
    ...
    return function(state)
        active = state
        print(name.." "..(state and "ativado!" or "desativado!"))
    end
end
...
protec:AddToggle({
    ...
    Callback = AntiFlingLoop("Canoes", function()
        ...
        return ws and ws:FindFirstChild("001_CanoeStorage")
    end)
})
...
protec:AddToggle({
    ...
    Callback = function(state)
        ...
        if state then
            ...
            print("Portas escondidas: " .. #_G.hiddenDoors)
        else
            ...
            print("Portas restauradas: " .. #(_G.hiddenDoors or {}))
            _G.hiddenDoors = {}
        end
    end
})
